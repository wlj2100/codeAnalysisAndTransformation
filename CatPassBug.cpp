...
// function to generate gen & kill set for one instruction, return a pair of sets
    ...
    bool runOnFunction (Function &F) override {
      ...
      std::vector<std::map<int, std::set<Instruction *>>> allVec;
      // [0:gen, 1:kill, 2:in, 3:out]
      int indexValue = 0;
      for (auto& B : F) {
        for (auto& I : B) {
         insV.push_back(&I);
          ...
          std::map<int, std::set<Instruction *>> tempMap;
          tempMap[0] = genKillPair.first;
          tempMap[1] = genKillPair.second;
          tempMap[2] = std::set<Instruction *>();
          tempMap[3] = genKillPair.first;
          allVec.push_back(tempMap);
      ...
      bool change=true, firstTime=true, blockReworkFlag=false, instReworkFlag=false;
      while(change) {
        int insIndex = 0;
        change = false;
        for (auto& B : F) {
          ...
          for (auto& I : B) {
            ...
            instReworkFlag = false;
            // inset
            // tempInSet
            auto* all = &allVec[insIndex];
            std::set<Instruction *>* tempInSetPtr = &((*all)[2]);
                ...
                for (auto PI = pred_begin(&B), E = pred_end(&B); PI != E; ++PI) {
                  ...
                  if (Pred != NULL) {
                    ...
                    if (preIn != NULL) {
                      ...
                      tempInSetPtr->insert(allVec[preinsIndex][3].begin(), allVec[preinsIndex][3].end());
                    }
                  }
                }
              } else {
            ...
            // inMap[insIndex] = tempInSet;
            // outset
            std::set<Instruction *> tempOutSet = (*all)[0];
            // tempOutSet.insert(genMap[insIndex].begin(), genMap[insIndex].end());
            std::set_difference(tempInSetPtr->begin(), tempInSetPtr->end(), (*all)[1].begin(), (*all)[1].end(), std::inserter(tempOutSet, tempOutSet.end()));
            if (tempOutSet != (*all)[3]) {
              change = true;
